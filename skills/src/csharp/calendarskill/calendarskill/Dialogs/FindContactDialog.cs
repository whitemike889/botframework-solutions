    ...
    public class FindContactDialog : CalendarSkillDialogBase
    {
        ...
        public async Task<DialogTurnResult> AfterUpdateUserName(WaterfallStepContext sc, CancellationToken cancellationToken = default(CancellationToken))
        {
            try
            {
                ...
                {
                    var lgResult = await _lgMultiLangEngine.Generate(sc.Context, "[UserNotFoundAgain]", new { userName = dialogState.FindContactInfor.CurrentContactName, source = userState.EventSource == Models.EventSource.Microsoft ? "Outlook Calendar" : "Google Calendar" });
                    var prompt = await new TextMessageActivityGenerator().CreateActivityFromText(sc.Context, lgResult, null);
                    await sc.Context.SendActivityAsync(prompt);
